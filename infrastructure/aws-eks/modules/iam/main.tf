# IAM Module for PyAirtable EKS Infrastructure
# Comprehensive IAM setup for EKS cluster, nodes, and IRSA
...
resource "aws_iam_role" "cluster_service_role" {
  name               = "${local.name}-eks-cluster-service-role"
...
# Additional cluster permissions for enhanced functionality
resource "aws_iam_role_policy" "cluster_additional_policy" {
  name = "${local.name}-eks-cluster-additional-policy"
...
resource "aws_iam_role" "node_instance_role" {
  name               = "${local.name}-eks-node-instance-role"
...
resource "aws_iam_role_policy_attachment" "node_instance_role_policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
  role       = aws_iam_role.node_instance_role.name
}

resource "aws_iam_role_policy_attachment" "node_instance_role_cni_policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
  role       = aws_iam_role.node_instance_role.name
}

resource "aws_iam_role_policy_attachment" "node_instance_role_registry_policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryReadOnly"
  role       = aws_iam_role.node_instance_role.name
}

# Additional node permissions
resource "aws_iam_role_policy" "node_additional_policy" {
  name = "${local.name}-eks-node-additional-policy"
...
# IRSA Roles for CSI Drivers and other services

# VPC CNI IRSA Role
resource "aws_iam_role" "vpc_cni_irsa" {
  name = "${local.name}-eks-vpc-cni-irsa"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(var.oidc_provider_arn, "/^(.*provider/)/", "")}:sub" = "system:serviceaccount:kube-system:aws-node"
...
resource "aws_iam_role" "ebs_csi_irsa" {
  name = "${local.name}-eks-ebs-csi-irsa"
...
resource "aws_iam_role" "efs_csi_irsa" {
  name = "${local.name}-eks-efs-csi-irsa"
...
resource "aws_iam_role_policy" "efs_csi_irsa_policy" {
  name = "${local.name}-eks-efs-csi-policy"
  role = aws_iam_role.efs_csi_irsa.id
...
resource "aws_iam_role" "cluster_autoscaler_irsa" {
  name = "${local.name}-eks-cluster-autoscaler-irsa"
...
resource "aws_iam_role" "load_balancer_controller_irsa" {
  name = "${local.name}-eks-load-balancer-controller-irsa"
...
resource "aws_iam_role_policy" "load_balancer_controller_policy" {
  name = "${local.name}-eks-load-balancer-controller-policy"
  role = aws_iam_role.load_balancer_controller_irsa.id
...
resource "aws_iam_role" "external_secrets_irsa" {
  name = "${local.name}-eks-external-secrets-irsa"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(var.oidc_provider_arn, "/^(.*provider/)/", "")}:sub" = "system:serviceaccount:external-secrets-system:external-secrets"
            "${replace(var.oidc_provider_arn, "/^(.*provider/)/", "")}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_role_policy" "external_secrets_policy" {
  name = "${local.name}-eks-external-secrets-policy"
...
resource "aws_iam_role" "cloudwatch_agent_irsa" {
  name = "${local.name}-eks-cloudwatch-agent-irsa"
