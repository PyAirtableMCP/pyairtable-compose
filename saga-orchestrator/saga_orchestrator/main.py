...
# Metrics - Initialize conditionally to avoid duplicate registration
try:
    ...
    saga_counter = None
    saga_duration = None  
    saga_step_duration = None
    active_sagas = None
    compensation_counter = None
    for collector in REGISTRY._collector_to_names.keys():
        if hasattr(collector, '_name'):
            if collector._name == 'saga_transactions_total':
                saga_counter = collector
            elif collector._name == 'saga_duration_seconds':
                saga_duration = collector
            elif collector._name == 'saga_step_duration_seconds':
                saga_step_duration = collector
            elif collector._name == 'active_sagas':
                active_sagas = collector
            elif collector._name == 'saga_compensations_total':
                compensation_counter = collector
...
app = FastAPI(
    ...
    description="Distributed transaction coordination service with comprehensive patterns",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health_check():
    ...
    return HealthResponse(
        status="healthy" if redis_status == "healthy" and http_status == "healthy" else "degraded",
...
async def root():
    ...
    return {
        ...
        "features": ["compensation", "timeout_handling", "retry_logic", "monitoring"]
    }


@app.post("/saga/start", response_model=SagaResponse)
async def start_saga(
    ...
    try:
        # Convert request steps to SagaStep objects
        steps = [
            SagaStep(
                step_id=step.step_id,
        ...
        logger.error("Failed to create SAGA", saga_id=request.saga_id, error=str(e))
        raise HTTPException(status_code=500, detail=f"Failed to create SAGA: {str(e)}")


@app.get("/saga/{saga_id}/status", response_model=SagaResponse)
async def get_saga_status(
    ...
    """Get SAGA status"""
    try:
        saga = await load_saga(saga_id, redis_conn)
...
@app.get("/saga/{saga_id}/steps")
async def get_saga_steps(
    saga_id: str,
    redis_conn: redis.Redis = Depends(get_redis)
):
    """Get detailed SAGA step information"""
    try:
        saga = await load_saga(saga_id, redis_conn)
        ...
        logger.error("Failed to get SAGA steps", saga_id=saga_id, error=str(e))
        raise HTTPException(status_code=500, detail=f"Failed to get SAGA steps: {str(e)}")


@app.post("/saga/{saga_id}/compensate")
async def manual_compensate_saga(
    saga_id: str,
    ...
    try:
        ...
        if saga.status in [SagaStatus.COMPENSATED, SagaStatus.COMPENSATING]:
            raise HTTPException(status_code=400, detail="SAGA already compensated or compensating")
        
        if saga.status == SagaStatus.RUNNING and not request.force:
            raise HTTPException(
                status_code=400, 
                detail="SAGA is still running. Use force=true to interrupt and compensate"
            )
        
        # Set compensation reason
        saga.compensation_reason = request.reason
        saga.status = SagaStatus.COMPENSATING
...
async def list_sagas(
    ...
    pattern: Optional[SagaPattern] = None,
    limit: int = 50,
    offset: int = 0,
    redis_conn: redis.Redis = Depends(get_redis)
):
    """List SAGA transactions with optional filtering"""
    try:
        saga_keys = await redis_conn.keys("saga:*")
        sagas = []
        
        for key in saga_keys[offset:offset + limit]:
            saga_id = key.split(":")[1]
            saga = await load_saga(saga_id, redis_conn)
            ...
            if status and saga.status != status:
                continue
            if pattern and saga.pattern != pattern:
                continue
        ...
        logger.error("Failed to list SAGAs", error=str(e))
...
async def get_metrics(redis_conn: redis.Redis = Depends(get_redis)):
    ...
    try:
        ...
        for key in saga_keys:
            saga_data = await redis_conn.hgetall(key)
            if saga_data:
                status = saga_data.get("status", "unknown")
                pattern = saga_data.get("pattern", "orchestration")
...
@app.delete("/saga/{saga_id}")
async def delete_saga(
    saga_id: str,
    force: bool = False,
    redis_conn: redis.Redis = Depends(get_redis)
):
    """Delete a SAGA transaction"""
    try:
        saga = await load_saga(saga_id, redis_conn)
        ...
        if saga.status in [SagaStatus.RUNNING, SagaStatus.COMPENSATING] and not force:
            raise HTTPException(
                status_code=400,
                detail="Cannot delete active SAGA. Use force=true to force deletion"
            )
        
        await redis_conn.delete(f"saga:{saga_id}")
        
        logger.info("SAGA deleted", saga_id=saga_id, force=force)
        
        return {"message": "SAGA deleted successfully"}
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error("Failed to delete SAGA", saga_id=saga_id, error=str(e))
        raise HTTPException(status_code=500, detail=f"Failed to delete SAGA: {str(e)}")
        ...
        "templates": [
            {
                ...
                "description": "Complete user registration with rollback capabilities",
                "steps": 4,
                "estimated_duration": "2-5 minutes"
            },
            {
                "name": "workspace_creation", 
                "description": "Create workspace with Airtable base setup",
                "steps": 4,
                "estimated_duration": "5-15 minutes"
            },
            {
                "name": "data_sync",
                "description": "Synchronize data between Airtable and external systems",
                "steps": 5,
            ...
            },
            {
                ...
                "description": "Process webhook events with choreography pattern",
                "steps": 4,
...
async def start_workflow(
    ...
    try:
        ...
        workflow_config = get_workflow_template(workflow_type, **workflow_data)
        
        # Create SAGA request
        saga_request = SagaRequest(**workflow_config)
        
        # Convert to SAGA and start
        steps = [
            SagaStep(
                step_id=step.step_id,
                service_url=step.service_url,
        ...
        saga = SagaTransaction(
            ...
            pattern=saga_request.pattern,
            status=SagaStatus.PENDING,
            steps=steps,
            timeout=saga_request.timeout,
            metadata=saga_request.metadata
        )
        ...
        logger.error("Failed to start workflow", workflow_type=workflow_type, error=str(e))
        raise HTTPException(status_code=500, detail=f"Failed to start workflow: {str(e)}")


# Choreography pattern endpoints for event-driven coordination
...
async def handle_event(
    ...
    """Handle events for choreography pattern"""
    try:
        logger.info(
            "Event received",
        ...
        # Publish event to Redis streams for choreography
        ...
        logger.error("Failed to handle event", event_type=event_type, error=str(e))
